  ...
  <div>
    ...
    <div style="background-color: white">
      ...
      <p align="left" style="font-size:20px">
        <b>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&nbsp;&nbsp;新建供应商委托人</b>
      </p>
      <a-divider/>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          ...
          created: "/ywvendorclientmanage/ywVendorClientManage/create",
    ...
    mounted () {
      ...
      //     this.vendorId = res.result.vendorId ;
      ...
      httpAction(this.url.querySupplier,"","get").then((res)=>{
        if(res.success){
          console.log("-------------------这是供应商的名称-----------------------") 
          console.log(res)
          console.log("------------------------------------------")
          ...
          let result2 = [];
          for (var i = 0; i < len; i++) {
            result1[i] = res.result[i]['vendor_name'];
            
            gysall[res.result[i]['vendor_name']]=res.result[i]['vendor_name']
            gysPid[res.result[i]['vendor_id']]=res.result[i]['vendor_id']
            result2[i] = res.result[i]['vendor_id'];
          }
          this.gysItems= result1
          this.gysNo= result2
          console.log(this.gysNo)
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'clentScopeId','clientStatementId','vendorId','vendorName','payCode','client','startDate','endDate','clientIdCard','idCardIndate','telephone','notes','outOfDate','onlineFillIndate','saleAuth','pickUpAuth','lisDefaultClient','isRebateAuth','createBy','createTime','updateBy','updateTime'))
        })
      },
      close () {
        this.$emit('close');
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          ...
          values.vendorName= gysall[values.vendorName]
          ...
          if (!err) {
            ...
            formData['vendorId']= this.gysNo[0]
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'clentScopeId','clientStatementId','vendorId','vendorName','payCode','client','startDate','endDate','clientIdCard','idCardIndate','telephone','notes','outOfDate','onlineFillIndate','saleAuth','pickUpAuth','lisDefaultClient','isRebateAuth','createBy','createTime','updateBy','updateTime'))
      },
      Reset(){
        this.form.resetFields();
      }
